...
const animekai  = new ANIME.AnimeKai();       // HiAnime (anciennement Zoro/Aniwatch)
...
const PROVIDERS: ProviderEntry[] = [
  ...
  { name: 'AnimeKai',  instance: animekai,  supportsVF: false, supportsVOSTFR: true, priority: 2 },
  ...
  for (const provider of orderedProviders) {
    try {
      ...
      if (provider.name === 'HiAnime') {
        sourceData = await resolveHiAnime(provider.instance as InstanceType<typeof ANIME.Hianime>, episodeId, lang);
      } else {
...
async function resolveHiAnime(
  instance: InstanceType<typeof ANIME.Hianime>,
  episodeId: string,
  lang: 'vf' | 'vostfr' | 'vo'
